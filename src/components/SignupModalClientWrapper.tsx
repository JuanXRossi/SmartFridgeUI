      ...
      {showVerification && (
        <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-50">
          <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-100 text-emerald-800 px-4 py-2 rounded-md shadow">
            <span>Thanks — check your email to verify your account</span>
            <button className="ml-3 text-slate-600" aria-label="Dismiss" onClick={() => setShowVerification(false)}>✕</button>
          </div>
        </div>
      )}
    </>
